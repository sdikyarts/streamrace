...
function getSourceArtistKey(row: ParsedAllCreditsRow) {
  if (row.spotifyArtistId) {
    return `spotify:${row.spotifyArtistId}`;
  }

  if (row.sourceUrl) {
    return `source:${row.sourceUrl}`;
  }
...
async function createImportRun({
  ...
  const [run] = await db
    ...
    .values({
      ...
      sourceTable: ALL_CREDITS_SOURCE_TABLE,
      sourceUrl,
      sourceDate,
      status: "running",
      collectionMethod,
...
async function rejectImportRun({
  ...
  await db
    ...
    .set({
      ...
      errorMessage: "All-credits import validation failed.",
      finishedAt: sql`now()`,
    })
...
function toArtistInsert(row: ParsedAllCreditsRowWithLeadRank): NewArtist {
  return {
    ...
    sourceArtistKey: getSourceArtistKey(row),
    displayName: row.artistName,
    sourceArtistName: row.artistName,
...
async function upsertArtists(
  ...
  const upsertedArtists = await tx
    ...
    .onConflictDoUpdate({
      target: artists.sourceArtistKey,
      set: {
        spotifyArtistId: excluded("spotify_artist_id"),
        sourceArtistName: excluded("source_artist_name"),
    ...
    .returning({
      id: artists.id,
      sourceArtistKey: artists.sourceArtistKey,
    });
  const idByArtistKey = new Map(
    upsertedArtists.map((artist) => [
      artist.sourceArtistKey,
      artist.id,
    ]),
  );

  return rows.map((row) => {
    const artistId = idByArtistKey.get(getSourceArtistKey(row));
...
export async function importAllCreditsRows({
  ...
  sourceDate,
  collectionMethod,
  sourceUrl = ALL_CREDITS_SOURCE_URL,
