...
function getSpotifyArtistId(sourceUrl: string | null) {
  if (!sourceUrl) {
    return null;
  }

  try {
    return new URL(sourceUrl).searchParams.get("artist_spotify_id");
  } catch {
    return null;
  }
}
...
function parseArtistCell(artistCell: string): ParsedArtistCell {
  ...
  return {
    ...
    sourceUrl: artistCell.slice(linkUrlStart + 1, linkUrlEnd),
